...
describe('fetchOidcJwksUri', (test) => {
  test('success', async () => {
    ...
    expect(fetch).toHaveBeenLastCalledWith('https://example.com/.well-known/openid-configuration');
    expect(uri).toBe('https://example.com/jwks.json');
  });

  test('not ok', async () => {
    fetchMock.mockResponseOnce(() => ({ status: 404 }));
    const uri = await fetchOIDCJwksUri('https://example.com/.well-known');
    expect(uri).toBe('https://example.com/.well-known/jwks.json');
  });
