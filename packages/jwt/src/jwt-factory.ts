...
import { type Jwk, type JwtHeader, type JwtIssuer, type JwtPayload, type PartialCrypto } from '@dreamauth/types';
...
export class JwtFactory {
  ...
  async create(
    ...
    if (!(jwk.alg in PARAMS)) return error('UnsupportedJwtAlg');
    ...
    const payloadFinal: JwtPayload = {
      ...
      jti: payload.jti ?? this.#crypto.randomUUID(),
