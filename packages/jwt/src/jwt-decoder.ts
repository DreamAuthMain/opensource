...
import { type JwkLoader, JwkOIDCLoader } from '@dreamauth/jwk';
...
export class JwtDecoder {
  ...
  constructor({ loader = new JwkOIDCLoader(), verifier }: JwtDecoderOptions = {}) {
    this.#loader = loader;
  ...
  async decode(value: string): Promise<Jwt> {
    ...
    await this.#verifier?.verify(jwt, this.#loader);
